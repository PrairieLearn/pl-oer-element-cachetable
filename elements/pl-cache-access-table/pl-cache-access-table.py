    ...
    for i in range(len(accesses)):
        if not isinstance(accesses[i]["address"], str):
            raise AttributeError(
                'Addresses need to be a string in data["correct_answers"]'
            )
...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    if (
        grade_mode != "through-first"
        and grade_mode != "all"
        and grade_mode != "all-or-nothing"
    ):
        raise AttributeError(
            'grade-mode must be "through-first" or "all" or "all-or-nothing'
        )
    ...
    for i in range(len(accesses)):
        ...
        if hit_score is not None and show_partial_score:
...
def grade(element_html: str, data: pl.QuestionData) -> None:
    ...
    for i in range(num_access):
        ...
        if a_ans == a_sub:
            ...
            num_correct += 1
        elif first_error is None and grade_mode == "through-first":
            data["partial_scores"][f"{name}{i}_hit"] = {
                "score": 0,
                "weight": 0,
            }
    ...
    if first_error is not None and grade_mode == "through-first":
